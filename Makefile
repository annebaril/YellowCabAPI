...
gcp_build:
	...
	docker build -t ${LOCATION}-docker.pkg.dev/${PROJECT_ID}/${REPOSITORY}/${IMAGE} flask_app 
...
deploy_service:
  ...
  --platform=managed --region=${LOCATION}-west1 --allow-unauthenticated

deploy: gcp_build docker_auth docker_push deploy_service #gcp_auth
### STRIP_END ###
